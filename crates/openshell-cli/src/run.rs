...
use openshell_core::proto::{
    ...
    BindMount, PortBinding, RevokeSshSessionRequest, RotateProviderCredentialRequest, Sandbox,
...
pub async fn sandbox_create(
    ...
    let port_bindings_vec = port_bindings.to_vec();
    let bind_mounts_vec = bind_mounts.to_vec();
    let template = if image.is_some()
        || resource_limits.is_some()
        || !port_bindings_vec.is_empty()
        || !bind_mounts_vec.is_empty()
    {
        Some(SandboxTemplate {
            image: image.unwrap_or_default(),
            resources: resource_limits,
            port_bindings: port_bindings_vec,
            bind_mounts: bind_mounts_vec,
...
mod tests {
    use super::{
        ...
        parse_credential_pairs, parse_port_binding, plaintext_gateway_is_remote,
